    ...
    public partial class Sales : Form
    {
           ...
           //int? tableId = (int?)cboTable.SelectedValue > 0 ? (int?)cboTable.SelectedValue : SettingController.UseQueue && !saveDraft ? (int?)Convert.ToInt16(txtNextQue.Text) : null;
            ...
            if (dgvSalesItem.Rows.Count > 1)
            {
                ...
                if (cusId > 0)
                {
                    ...
                    Id = entity.InsertDraft(DateTime.Now, MemberShip.UserId, MemberShip.CounterId, TransactionType.Sale, true, true, Convert.ToInt32(cboPaymentMethod.SelectedValue), tax, extraDiscount, DetailList.Sum(x => x.TotalAmount) + tax - extraDiscount, null, null, cusId, IsWholeSale, 0, 0);
                }
                else
                {

                    Id = entity.InsertDraft(DateTime.Now, MemberShip.UserId, MemberShip.CounterId, TransactionType.Sale, true, true, Convert.ToInt32(cboPaymentMethod.SelectedValue), tax, extraDiscount, DetailList.Sum(x => x.TotalAmount) + tax - extraDiscount, null, null, null, IsWholeSale, 0, 0);
                }
                entity = new POSEntities();
        ...
        private void dgvSalesItem_CellLeave(object sender, DataGridViewCellEventArgs e)
        {
            if (e.RowIndex >= 0)
            {
                DataGridViewRow row = dgvSalesItem.Rows[e.RowIndex];
                dgvSalesItem.CommitEdit(new DataGridViewDataErrorContexts());
                if (row.Cells[0].Value == null && row.Cells[1].Value == null && row.Cells[2].Value == null && row.Cells[5].Value == null && row.Cells[3].Value == null && row.Cells[4].Value == null)
                {
        ...
        private void dgvSalesItem_EditingControlShowing1(object sender, DataGridViewEditingControlShowingEventArgs e)
        {
            ...
            if (dgvSalesItem.CurrentCell.OwningColumn.Name.Equals("colProductName"))
            {
                ...
                prodCode.Font= new Font("Zawgyi-One", 10,FontStyle.Regular);
                if (prodCode != null)
                {
