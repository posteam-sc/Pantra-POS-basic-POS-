...
namespace POS
{
    public partial class PurchaseInput : Form
    {
        ...
        private void Calcuate_PayableAmount()
        {
            ...
            int totalQtyCount = 0;
            foreach (PurchaseProductController ppc in PurchaseProductList)
            {
                totalQtyCount += ppc.Qty;
            }
            txtTotalQty.Text = totalQtyCount.ToString();
            if (PurchaseProductList != null)
            {
                txtNoOfItems.Text = PurchaseProductList.Count.ToString();
            }
            else
            {
                txtNoOfItems.Text = "0";
