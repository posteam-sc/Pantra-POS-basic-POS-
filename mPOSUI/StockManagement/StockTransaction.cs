...
namespace POS
{
    public partial class StockTransaction : Form
    {
        ...
        private void btnProcess_Click(object sender, EventArgs e)
        {
            ...
            //Cursor.Current = Cursors.WaitCursor;
            //productIdList = (from p in entity.Products select p.Id).ToList();
            //DateTime dt = DateTime.Now.AddMonths(-17);

            //while (dt < DateTime.Now.AddMonths(-1))
            //{
            //    if (Utility.Stock_Transaction_Process(dt.Year, dt.Month, productIdList))
            //    {
            //        label3.Text = dt.ToString("MM-yy");
            //        label3.Refresh();
            //    }
            //    dt = dt.AddMonths(1);
            //}
